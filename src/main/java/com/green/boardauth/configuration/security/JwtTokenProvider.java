...
public class JwtTokenProvider {
    ...
    //JWT 문자열을 만드는 메소드, 암호화된 문자열(데이터, 토큰만료시간)
    public String generateToken(JwtUser jwtUser, long tokenValidityMilliSeconds) {
        Date now = new Date(); // import java.util.Date; 임포트하기
        return Jwts.builder()
            //header
            ...
            .expiration( new Date(now.getTime() + tokenValidityMilliSeconds) ) //JWT종료 일시 (토큰 만료일시) (중요)
            .claim(constJwt.getClaimKey(), makeClaimByUserToJson(jwtUser) ) //claim에 담는 (key, value)
                                                                            // signedUser 키값으로 JwtUser객체를 JSON으로 변환하여 담았다.
    ...
    public JwtUser getJwtUserFromToken(String token) {
        ...
        //signedUser 키값으로 담겨져있는 value를 String타입으로 리턴해줘~
