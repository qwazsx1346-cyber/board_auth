...
public class BoardController {
    ...
    @GetMapping("{id}")
    public BoardGetOneRes getBoardDetail(@PathVariable long id) {
        log.info("id: {}", id);
        return boardService.getBoardDetail(id);
    }

    @PutMapping
    public ResultResponse<?> putBoard(@AuthenticationPrincipal UserPrincipal userPrincipal, @RequestBody BoardPostPutReq req) {
        req.setUserId( userPrincipal.getSignedUserId() );
        log.info("req: {}", req);
    ...
    @DeleteMapping
    public ResultResponse<?> delList(@AuthenticationPrincipal UserPrincipal userPrincipal
